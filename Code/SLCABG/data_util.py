...
def get_txt(str):
    ...
    sentences = []
    rs_sentences = []
    with open('data/' + str + '.txt', 'r', encoding='utf-8') as fp:  # 加载正向评论
        lines = fp.readlines()
        for line in lines:
            sentences.append(line.strip())
    jieba.load_userdict(sentiment_dict.keys())  # 以情感词典中的keys进行jieba分词
    for sentence in sentences:
        sentence = list(jieba.cut(sentence))
        split_sentence = []
        for word in sentence:
            if '\u4e00' <= word <= '\u9fff' and word not in stopwords:  # 判断是否为中文字符以及是否属于停用词
                split_sentence.append(word)
        rs_sentences.append(split_sentence)
    sentences = rs_sentences
    with open('data/' + str + '_word_list.txt', 'w', encoding='utf-8') as fp:
        for sentence in sentences:
...
#     for word in words_list:
#         fp.write(word + '\n')
# np.save('data/wordsList.npy', np.array(words_list))
# word2vec_original = get_word_vectors()
# word2vec = word2vec_original
# sentiment_dict = get_sentiment_dict()
# for i in word2vec.keys():
#     if i in sentiment_dict.keys():
#         word2vec[i] = sentiment_dict[i] * word2vec[i]