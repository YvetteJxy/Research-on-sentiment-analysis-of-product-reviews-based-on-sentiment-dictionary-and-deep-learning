...
if __name__ == '__main__':
    ...
    train_data_loader = torch.utils.data.DataLoader(MyData(x_train, y_train), 32, True, drop_last=True)
    test_data_loader = torch.utils.data.DataLoader(MyData(x_test, y_test), 32, False, drop_last=True)

    config=Config()
    net = RNN( word_vectors,config).to(device)
    ...
    for epoch in range(15):
        for i, (cls, sentences) in enumerate(train_data_loader):
            ...
            loss = criterion(out, cls).to(device) #这里会报batch size错误
            loss.backward()
            optimizer.step()
            if (i + 1) % 1 == 0:
